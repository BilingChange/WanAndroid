...
abstract class BaseMultiTypeAdapter @JvmOverloads constructor(
    ...
    open var types: com.drakeet.multitype.Types = com.drakeet.multitype.MutableTypes(initialCapacity),
     ...
     * */
    fun <T> register(clazz: Class<T>, delegate: com.drakeet.multitype.ItemViewBaseDelegate<T, *>) {
        unregisterAllTypesIfNeeded(clazz)
        register(com.drakeet.multitype.Type(clazz, delegate, com.drakeet.multitype.DefaultLinker()))
    }

    inline fun <reified T : Any> register(delegate: com.drakeet.multitype.ItemViewBaseDelegate<T, *>) {
    ...
    inline fun <reified T : Any> register(
        ...
        delegate: com.drakeet.multitype.ItemViewBaseDelegate<T, *>,
    ) {
    ...
    fun <T> register(clazz: Class<T>, binder: com.drakeet.multitype.ItemViewBinder<T, *>) {
        register(clazz, binder as com.drakeet.multitype.ItemViewBaseDelegate<T, *>)
    }

    fun <T> register(clazz: Class<T>, binder: com.drakeet.multitype.PagingItemViewBinder<T, *>) {
        register(clazz, binder as com.drakeet.multitype.ItemViewBaseDelegate<T, *>)
    }

    inline fun <reified T : Any> register(binder: com.drakeet.multitype.ItemViewBinder<T, *>) {
        register(binder as com.drakeet.multitype.ItemViewBaseDelegate<T, *>)
    }

    inline fun <reified T : Any> register(binder: com.drakeet.multitype.PagingItemViewBinder<T, *>) {
        register(binder as com.drakeet.multitype.ItemViewBaseDelegate<T, *>)
    }

    inline fun <reified T : Any> register(clazz: KClass<T>, binder: com.drakeet.multitype.ItemViewBinder<T, *>) {
        register(clazz, binder as com.drakeet.multitype.ItemViewBaseDelegate<T, *>)
    }

    inline fun <reified T : Any> register(clazz: KClass<T>, binder: com.drakeet.multitype.PagingItemViewBinder<T, *>) {
        register(clazz, binder as com.drakeet.multitype.ItemViewBaseDelegate<T, *>)
    }

    internal fun <T> register(type: com.drakeet.multitype.Type<T>) {
        types.register(type)
        when (type.delegate) {
            is com.drakeet.multitype.ItemViewDelegate -> {
                check(this is com.drakeet.multitype.MultiTypeAdapter) { IllegalArgumentException("Are you register ${type.delegate} to MultiTypeAdapter correctly?") }
                type.delegate._adapter = this
            }
            is com.drakeet.multitype.PagingItemViewDelegate -> {
                check(this is com.drakeet.multitype.PagingMultiTypeAdapter<*>) { IllegalArgumentException("Are you register ${type.delegate} to PagingMultiTypeAdapter correctly?") }
                type.delegate._adapter = this
            }
        }
    }
     ...
     */
    @CheckResult
    fun <T> register(clazz: Class<T>): com.drakeet.multitype.OneToManyFlow<T> {
        unregisterAllTypesIfNeeded(clazz)
        return com.drakeet.multitype.OneToManyBuilder(this, clazz)
    }

    @CheckResult
    fun <T : Any> register(clazz: KClass<T>): com.drakeet.multitype.OneToManyFlow<T> {
     ...
     * @see [register]
     */
    fun registerAll(types: com.drakeet.multitype.Types) {
        val size = types.size
    ...
    private fun getOutDelegateByViewHolder(holder: RecyclerView.ViewHolder): com.drakeet.multitype.ItemViewBaseDelegate<Any, RecyclerView.ViewHolder> {
        @Suppress("UNCHECKED_CAST")
        return types.getType<Any>(holder.itemViewType).delegate as com.drakeet.multitype.ItemViewBaseDelegate<Any, RecyclerView.ViewHolder>
    }

    private fun getOutDelegateByViewHolder(position: Int): com.drakeet.multitype.ItemViewBaseDelegate<Any, RecyclerView.ViewHolder> {
        @Suppress("UNCHECKED_CAST")
        return types.getType<Any>(getItemViewType(position)).delegate as com.drakeet.multitype.ItemViewBaseDelegate<Any, RecyclerView.ViewHolder>
    }

    @Throws(com.drakeet.multitype.DelegateNotFoundException::class)
    internal fun indexInTypesOf(position: Int, item: Any): Int {
        ...
        throw com.drakeet.multitype.DelegateNotFoundException(item.javaClass)
    }

    private fun unregisterAllTypesIfNeeded(clazz: Class<*>) {
        if (types.unregister(clazz)) {
            Log.w(com.drakeet.multitype.BaseMultiTypeAdapter.Companion.TAG, "The type ${clazz.simpleName} you originally registered is now overwritten.")
        }
    }
